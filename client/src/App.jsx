...
export default function App() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((r) => {
        ...
        setCanRunQueries(r.data.canRunQueries === true);
      })
      .catch((e) => {
  ...
  useEffect(() => {
    ...
    setSchema(null);
    api
      .get(`/tables/${encodeURIComponent(selected)}/schema`)
      .then((r) =>
        setSchema({
  ...
  useEffect(() => {
    if (selected && user) loadData();
  }, [selected, page, pageSize, sort, user]);
  ...
  async function loadData(customSearch = search) {
    ...
    setError("");
    try {
      const r = await api.get(`/tables/${encodeURIComponent(selected)}/rows`, {
        params: {
          page,
          pageSize,
          search: customSearch,
  ...
  async function save(values) {
    try {
      if (modal.mode === "insert")
        await api.post(`/tables/${encodeURIComponent(selected)}/rows`, {
          values,
        });
      else {
        const keyColumn = schema.primary_key[0];
        await api.put(`/tables/${encodeURIComponent(selected)}/rows`, {
          keyColumn,
          keyValue: modal.row[keyColumn],
          values,
        });
      }
      setModal(null);
    ...
    <div className="app-shell">
      ...
      <main>
        <header>
          <div>
            <h1>MySQL Data Manager</h1>
            <p>{selected || "Select a table"}</p>
          </div>
          {schema && (
            <button onClick={() => setModal({ mode: "insert" })}>
              + Add Record
            </button>
          )}
        ...
        <nav className="query-actions" aria-label="Workspace">
          <button
            className={view === "tables" ? "primary" : "secondary"}
            onClick={() => {
              setView("tables");
              loadData();
            }}
          ...
          <button
            className={view === "query" ? "primary" : "secondary"}
        ...
        <div hidden={view !== "query"}>
          <QueryConsole key={user} allowed={canRunQueries} />
        </div>
        {view === "tables" && (
          <>
            <div className="toolbar">
              <input
                ...
                onKeyDown={(e) =>
                  e.key === "Enter" && (setPage(1), loadData(search))
                }
                  ...
                  <thead>
                    <tr>
                      {schema?.columns.map((c) => (
                        <th key={c.name} onClick={() => changeSort(c.name)}>
                          {c.name}
                          {sort.column === c.name
                            ? sort.direction === "asc"
                              ? " ▲"
                    ...
                    {rows.map((row, i) => (
                      <tr key={i}>
                        ...
                        <td>
                          <button
                            className="small"
                            disabled={schema?.primary_key?.length !== 1}
