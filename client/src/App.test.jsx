...
vi.mock('./services/api', () => ({
  ...
  request: vi.fn(),
  onUnauthorized: listener => { state.listeners.add(listener); return () => state.listeners.delete(listener) },
}))
const schema = [
  { name: 'id', columnKey: 'PRI', dataType: 'int', columnType: 'int', extra: 'auto_increment' },
  { name: 'name', columnKey: '', dataType: 'varchar', columnType: 'varchar(50)', extra: '' },
]
beforeEach(() => {
  state.superUser = true
  api.get.mockImplementation(async url => ({ data: url === '/auth/me'
    ? { username: 'tester', isSuperUser: state.superUser, canRunQueries: true }
    : [{ name: 'people' }] }))
  request.mockImplementation(async url => url.endsWith('/schema') ? schema : {
    data: [{ id: 1, name: 'Alice' }], pagination: { page: 1, total: 1, totalPages: 1 },
  })
  api.post.mockResolvedValue({ data: { durationMs: 1, results: [{ columns: ['id', 'name'], rows: [[1, 'Alice']], edit: { table: 'people', keyColumn: 'id', columns: ['id', 'name'] } }] } })
...
test('active entry supports table search and super-user editing under StrictMode', async () => {
  ...
  await waitFor(() => expect(request.mock.calls.some(([url]) => url.includes('search=Alice'))).toBe(true))
...
test('SQL editor runs, saves by original primary key, and retains the draft across views', async () => {
  ...
  await waitFor(() => expect(api.put).toHaveBeenCalledWith('/tables/people/rows', { keyColumn: 'id', keyValue: 1, values: { name: 'Bob' } }))
