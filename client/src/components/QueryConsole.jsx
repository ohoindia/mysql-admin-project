...
export default function QueryConsole({ allowed, superUser, onUnauthorized }) {
  ...
  async function saveEdit(event) {
    ...
    try {
      ...
      const changes = Object.fromEntries(
        Object.entries(values).filter(([name]) => name !== set.edit.keyColumn),
      );
  ...
  async function run() {
    ...
    try {
      ...
      if (e.response?.status === 401) onUnauthorized?.();
      setError(
        `${e.response?.data?.code ? `${e.response.data.code}: ` : ""}${e.response?.data?.error || e.message}`,
      );
    } finally {
      busy.current = false;
      setRunning(false);
  ...
  return (
    <section className="query-console" aria-label="SQL Console">
      <p>
        Run one SQL statement, or highlight the statement to execute.
        Ctrl/Cmd+Enter runs it.
      </p>
      <p>
        Changes are committed immediately. Each run uses a fresh connection;
        transactions cannot span runs. Use LIMIT for large queries; results
        display up to 1,000 rows per result set.
      </p>
      {!allowed && (
        <div className="error-msg">
          SQL Console requires unrestricted table access. Sign in as the super
          user.
        </div>
      )}
      ...
      <div className="query-actions">
        <button
          className="primary"
          disabled={running || !allowed || !sql.trim()}
          onClick={run}
        >
      ...
      {result?.results.map((set, index) => (
        <section key={index} aria-label={`Result ${index + 1}`}>
          {set.columns ? (
            <>
              <p role="status">
                {set.rows.length} rows returned
                {set.truncated
              ...
              <div className="query-results" tabIndex={0}>
                <table>
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    {set.rows.map((row, i) => (
                      <tr key={i}>
                        {row.map((value, j) => (
                          <td key={j} title={displayValue(value)}>
                            {value === null ? (
                ...
                {!set.rows.length && (
                  <p className="no-data">Query succeeded. No rows returned.</p>
                )}
              </div>
            </>
          ) : (
            <div className="msg" role="status">
              Statement succeeded. {set.affectedRows} rows affected.
              {set.insertId ? ` Insert ID: ${set.insertId}.` : ""}{" "}
              {set.warningCount} warnings. {set.info}
            </div>
          )}
        </section>
      ))}
      {superUser && editing && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="query-edit-title"
        >
          <form className="card" onSubmit={saveEdit}>
            <div className="modal-header">
              <h3 id="query-edit-title">Edit {editing.set.edit.table}</h3>
            </div>
            <div className="modal-body">
              <div className="edit-grid">
                {editing.set.edit.columns.map((name) => (
                  <label key={name}>
                    {name}
                    <input
                      value={editing.values[name] ?? ""}
                      disabled={saving || name === editing.set.edit.keyColumn}
                      onChange={(e) =>
                        setEditing((current) => ({
                          ...current,
