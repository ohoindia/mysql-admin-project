...
test('late responses from an old table selection cannot replace the current results', async () => {
  const pending = []
  request.mockImplementation((url, options) => new Promise(resolve => pending.push({ url, options, resolve })))
  const { result, unmount } = renderHook(useTableBrowser)
  let first, second
  act(() => { first = result.current.selectTable('first') })
  act(() => { second = result.current.selectTable('second') })
  expect(pending[0].options.signal.aborted).toBe(true)
  await act(async () => {
    pending[2].resolve([{ name: 'id', columnKey: 'PRI' }])
    pending[3].resolve({ data: [{ id: 2 }], pagination: { page: 1, total: 1, totalPages: 1 } })
    await second
  })
  await act(async () => {
