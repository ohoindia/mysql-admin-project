...
require("mysql2/promise").createPool = () => ({
  getConnection: async () => ({
    ...
    async query(options) {
      ...
      if (options.sql === "UPDATE visible SET value = 2")
        return [{ affectedRows: 3, warningStatus: 0 }, undefined];
      if (options.sql === "SELECT large")
        return [Array.from({ length: 1001 }, (_, i) => [i]), [{ name: "id" }]];
  ...
  return handler(
    {
      ...
      cookies,
      requestContext: {
        http: { method, path, sourceIp: "127.0.0.1", protocol: "HTTP/1.1" },
      },
...
test("untrusted origins cannot log in or perform authenticated writes", async () => {
  for (const method of ["OPTIONS", "POST", "PUT", "DELETE"]) {
    const response = await invoke(method, "/api/auth/login", {
...
test("protected routes reject missing and tampered cookies", async () => {
  ...
  assert.equal(
    (
      await invoke("GET", "/api/auth/me", {
        cookies: ["mysql_admin_session=bad"],
      })
    ).statusCode,
    401,
  );
...
test("login returns a bearer token that authenticates API Gateway v2 requests without cookies", async () => {
  ...
  assert.equal(
    (
      await invoke("GET", "/api/auth/me", {
        cookies: [`mysql_admin_session=${token}`],
      })
    ).statusCode,
    401,
  );
...
test("expired, future, tampered, and legacy cookie tokens are rejected", async () => {
  ...
  for (const [time, prefix, secret] of [
    ...
    assert.equal(
      (await invoke("GET", "/api/auth/me", { token })).statusCode,
      401,
    );
  }
});
...
test("super user bypasses table restrictions while admin remains restricted", async () => {
  for (const username of ["admin", "superadmin"]) {
    ...
    assert.deepEqual(JSON.parse(me.body), {
      username,
      canRunQueries: username === "superadmin",
      isSuperUser: username === "superadmin",
    });
    const tables = await invoke("GET", "/api/tables", { token });
    assert.deepEqual(
      JSON.parse(tables.body),
      username === "admin"
        ? [{ name: "visible" }]
        : [{ name: "visible" }, { name: "restricted" }],
    );
    assert.equal(
      (await invoke("GET", "/api/tables/visible/schema", { token })).statusCode,
      200,
    );
    for (const [method, suffix, superStatus] of [
      ...
      const response = await invoke(
        method,
        `/api/tables/restricted/${suffix}`,
        { token },
      );
      assert.equal(
        response.statusCode,
        username === "admin" ? 403 : superStatus,
      );
...
test("SQL console enforces auth and table restrictions, validates input and preserves results", async () => {
  assert.equal(
    (await invoke("POST", "/api/query", { body: { sql: "SELECT 1" } }))
      .statusCode,
    401,
  );
  for (const username of ["admin", "superadmin"]) {
    ...
    const response = await invoke("POST", "/api/query", {
      token,
      body: { sql: "SELECT 1" },
    });
    ...
    assert.deepEqual(JSON.parse(response.body).results, [
      { columns: ["value", "value"], rows: [[1, null]], truncated: false },
    ]);
    for (const sql of ["", "  ", 42, "x".repeat(100001)]) {
      assert.equal(
        (await invoke("POST", "/api/query", { token, body: { sql } }))
          .statusCode,
        400,
      );
    }
    const writes = await invoke("POST", "/api/query", {
      token,
      body: { sql: "UPDATE visible SET value = 2" },
    });
    assert.equal(JSON.parse(writes.body).results[0].affectedRows, 3);
    const large = JSON.parse(
      (
        await invoke("POST", "/api/query", {
          token,
          body: { sql: "SELECT large" },
        })
      ).body,
    );
    ...
    const failure = await invoke("POST", "/api/query", {
      token,
      body: { sql: "bad SQL" },
    });
    ...
    const editable = await invoke("POST", "/api/query", {
      token,
      body: { sql: "SELECT id AS alias FROM editable" },
    });
    assert.deepEqual(JSON.parse(editable.body).results[0].edit, {
      table: "editable",
      keyColumn: "id",
      columns: ["id"],
    });
    const invalidKey = await invoke("PUT", "/api/tables/editable/rows", {
      token,
      body: { keyColumn: "other", keyValue: 1, values: { id: 2 } },
    });
